...
sudo python Tunnel_UART.py config/config_AuxUART.ini
...
def is_connected(_DEBUG_ON=False):
    ...
    try:
        response = os.system("ping -c 1 -w500 -W500 " + REMOTE_SERVER + " > /dev/null 2>&1")
        ...
        while(1):
            ...
            if((np.size(dataFails)!=0) & (Scheduler == 1)):
                if((is_connected() == True)):
                    ...
                    try:
                        if(verbose_on):
                            print("Data2Send: ", dataFails[numInternet-1])
            ...
            #Task Scheduler
            Scheduler = 1
            time.sleep(0.5) #TODO Test with diferents value
